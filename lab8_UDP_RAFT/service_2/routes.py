...
from __main__ import app, server
from app import *

@app.route(f'/credentials', methods=['GET'])
def credentials():
    if request.method == 'GET':
        if server.role == "leader":
            print(server.followers)
            return jsonify(server.followers), 200
        else:
            print(server.leader_data)
            return jsonify(server.leader_data), 200

@app.route('/api/electro-scooters', methods=['POST'])
def create_electro_scooter():
    if server.role != "leader" and ("Token" not in request.headers or request.headers["Token"] != "token"):
        return jsonify({"error": "Only the leader can create a new Electro Scooter"}), 401
    else:
        try:
            ...
            if server.role == "leader":
                print(server.followers)
                for follower in server.followers:
                    print(follower)
                    requests.post(f"http://{follower['host']}:{follower['port']}" + "/api/electro-scooters", json=data, headers={"Token": "token"})
        ...
        try:
            ...
            if scooter is not None:
                ...
                if server.role == "leader":
                    ...
                    for follower in server.followers:
                        print(follower)
                        requests.put(f"http://{follower['host']}:{follower['port']}" + "/api/electro-scooters/"+str(scooter_id), json=data, headers={"Token": "token"})
            ...
            # Check if the provided password is correct
                if password == 'your_secret_password': # Replace with your actual password
                    ...
                    if server.role == "leader":
                        ...
                        for follower in server.followers:
                            print(follower)
                            requests.delete(f"http://{follower['host']}:{follower['port']}" + "/api/electro-scooters/"+ str(scooter_id), headers={"Token": "token", "X-Delete-Password": "your_secret_password"})
